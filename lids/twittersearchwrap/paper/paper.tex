...
Topics trending worldwide and in Germany at Twitter retrieved at February 24th
...
of queries using the retrieved trending topics given in Table \ref{tbl:terms}
as input queries for the evaluation. The set of queries was executed twice per
hour in a 24 hour time frame starting from the moment the topics were identified
and retrieved as trending topics. In a first step we have filtered the result
sets such that each set is disjoint to all the other sets for the same query with respect to the messages it contains. We have then evaluated the results with respect to several measures:
