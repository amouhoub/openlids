...
\begin{itemize}
\item Temporal Sliding Window: The query is executed and the n most
recent results are retrieved every t minutes. The Temporal Sliding Window method
may lead to duplicate messages in the retrieved documents, but needs to handle much less data and
allows for an analysis over larger time frames.
\item k-item Sliding Window: The k-item Sliding Window methods retrieves all
messages that match the query exactly one time. Each time k new messages are
...
Using the retrieved trending topics given in Table \ref{tbl:terms}
as input queries for the evaluation, the set of queries was executed twice
per hour using the Temporal Sliding Window method for data acquisition in a 24
hour time frame starting from the moment the topics were identified as trending topics.  We have then evaluated the results with respect to several measures:
